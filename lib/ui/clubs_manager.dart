...
class ClubsManager extends StatefulWidget {
  @override
  _ClubsManagerState createState() => _ClubsManagerState();
}

class _ClubsManagerState extends State<ClubsManager> {
  BaseAuth _auth = Auth();
  ...
  void initState() {
    super.initState();
    _auth.getCurrentUser().then((FirebaseUser _user){
      setState(() {
        _userUid = _user.uid;
      });
    });
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: StreamBuilder(
        stream: Firestore.instance.collection('clubs').where('users', arrayContains: _userUid).snapshots(),
